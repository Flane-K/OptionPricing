...
def plot_3d(option_type):
    spot_range = np.linspace(0.5*S, 1.5*S, 40)
    ...
    fig = go.Figure(data=[go.Surface(x=Spot, y=Vol, z=Z, colorscale='Viridis')])
    fig.update_layout(title=f"{option_type.capitalize()} Option Surface", scene=dict(
        xaxis_title="Spot Price", yaxis_title="Volatility", zaxis_title="Option Price"))
    return fig
...
def plot_heatmap(option_type):
    ...
    fig, ax = plt.subplots()
    c = ax.imshow(Z, aspect='auto', cmap='viridis',
                  extent=[spot_range[0], spot_range[-1], vol_range[0], vol_range[-1]])
    ax.set_xlabel("Spot Price")
    ax.set_ylabel("Volatility")
    ax.set_title(f"{option_type.capitalize()} Option Heatmap")
