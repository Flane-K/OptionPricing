...
# This can be expanded with more tickers or replaced with a dynamic API call in the future.
MARKETS = {
    "🇺🇸 US Exchanges": {
        "currency": "$",
    ...
    "🇮🇳 Indian Exchanges": {
        "currency": "₹",
        "rf_ticker": "^CNXFIN", # Using a broad financial index as proxy, user can override.
        "rf_name": "India RBI Bond (Proxy)",
        "stocks": {
    ...
    "🇪🇺 European Exchanges": {
        "currency": "€",
        "rf_ticker": "^TREUR", # Using a generic Euro treasury rate if available, else default
        "rf_name": "Eurozone Bond Rate",
        "stocks": {
...
def bs_greeks(S, K, T, r, sigma, option_type="call"):
    ...
    if T == 0 or sigma == 0: return (1, 0, 0, 0, 0) if option_type == 'call' else (-1, 0, 0, 0, 0)
    d1 = (np.log(S / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
...
def mc_greeks(S, K, T, r, sigma, option_type="call", num_simulations=10000):
    ...
    dS = S * 0.01
    dT = T / 100 if T > 0 else 0.001
    d_sigma = sigma * 0.01
    d_r = r * 0.01 if r > 0 else 0.0001

    price_mid = monte_carlo_option_pricing(S, K, T, r, sigma, option_type, num_simulations)

    # Delta, Gamma, Vega, Rho... (finite differences logic as in binomial model)
    ...
    vega = (price_vol_up - price_mid) / d_sigma # Using one-sided diff for stability
    ...
    rho = (price_r_up - price_mid) / d_r # Using one-sided diff for stability
...
with st.sidebar.expander("📈 Underlying Stock Parameters", expanded=True):
    ...
    # 2. Stock Selection with search
    stock_options = ["Select a stock..."] + [f"{ticker} - {name}" for ticker, name in market_stocks.items()]
    selected_stock_formatted = st.selectbox(
        "Search for Stock (Ticker or Name)",
    ...
    if selected_stock_formatted != "Select a stock...":
        ...
        try:
            ...
            # Use a longer period for more robust data, then tail it
            ...
            if not hist.empty:
                ...
                vol_est = np.std(log_ret) * np.sqrt(252) # Annualized volatility
        ...
        try:
            rf_data = yf.Ticker(rf_ticker).history(period="5d")["Close"]
...
# ------------------- Main Logic to get prices and greeks -------------------
...
# Main calculation block, only runs if a stock is selected
if not ticker:
    ...
    with tab1:
        ...
        fig.update_layout(
            ...
            xaxis_title=f"Stock Price at Expiration ({currency})",
            yaxis_title=f"Profit / Loss per Share ({currency})",
    ...
    with tab2:
        ...
        # Displaying comparison in a clean way
        ...
        st.dataframe(pd.DataFrame(call_data).set_index("Metric"), use_container_width=True)
        st.subheader("Put Option Comparison")
        st.dataframe(pd.DataFrame(put_data).set_index("Metric"), use_container_width=True)


    with tab3:
        # 3D plot logic remains the same
    ...
    with tab4:
        ...
        def plot_plotly_heatmap(prices, spot_range, vol_range, title):
            fig = go.Figure(data=go.Heatmap(
                z=prices, x=[f"{s:.2f}" for s in spot_range], y=[f"{v:.2f}" for v in vol_range],
                hoverongaps=False, colorscale='viridis', text=np.around(prices, 2), texttemplate="%{text}"
            ))
    ...
    with tab5:
        # Sensitivity analysis logic remains the same
